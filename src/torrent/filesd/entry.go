...
// Entry wraps the EntryBase struct with the starting and ending
...
func (few *Entry) StartPiece() int64 {
	return few.startPieceIdx
}

func (few *Entry) EndPiece() int64 {
	return few.endPieceIdx
}

func (few *Entry) StartPieceOff() int64 {
	return few.startPieceOff
}

func (few *Entry) EndPieceOff() int64 {
...
func MakeEntryWrapper(en EntryBase, startIdx, endIdx, startOff, endOff int64) Entry {
	return Entry{
		EntryBase:     en,
...
// If the file contains no piece at the specified index, an error is returned.
func (few *Entry) PieceInfo(index int64, pieceLen int64) (PieceInfo, error) {
	/* ===================================================================
	...
	if index < few.startPieceIdx || index > few.endPieceIdx {
		return PieceInfo{}, fmt.Errorf("index %v out of range", index)
	}
